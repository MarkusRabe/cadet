...
void skolem_decision(Skolem* s, Lit decision_lit) {
    ...
    // Introduce a new SAT solver variable that represents the output of the Skolem function
...
//        satsolver_clause_finished(s->skolem);
    ...
    if (s->options->functional_synthesis) {
        ...
        // Define the decision_lit := new_val_satlit && -val_satlit  // - opposite_satlit && - val_satlit
...
//        skolem_update_depends_on_decision_satlit(s, decision_var_id, actual_depends_on_decision_satlit);
    ...
    // Decision variable needs to be deterministic before we can do conflict checks. This is why we have to check exactly here.
