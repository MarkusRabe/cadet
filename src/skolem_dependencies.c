...
bool skolem_has_illegal_dependence(Skolem* s, Clause* c) {
    if (s->qcnf->problem_type <= QCNF_2QBF) {
        ...
        Lit uc_lit = skolem_get_unique_consequence(s, c);
        for (int i = c->size - 1; i >= 0; i--) { // running backwards since illegal dependencies are more likely at the end of the clause
            if (c->occs[i] != uc_lit) {
...
bool skolem_var_has_empty_maximal_dependencies(QCNF* q, unsigned var_id) {
    ...
    return v->scope_id == qcnf_get_empty_scope(q);
}
